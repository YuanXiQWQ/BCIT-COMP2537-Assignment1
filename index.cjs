 ...
 * \w+[%$#&]\w+     Cannot start and end with one or more letter or number(\w+) while "%$#&" within.
 * .*\|\|.*         Cannot have "||".
 * \s+(?:and|or)\s+ Cannot have "and", "or" (single word, which space surrounds).
 * \b(...)\b        Cannot have these single word
 * invert: true     Cannot, otherwise description upper will become "Must have"
 *
